...
title: "[ARM32] 복귀주소를 자동 저장하는 BL 명령어 활용"
...
`B` 명령이 단순히 특정 레이블로 점프하는 역할이었다면, `BL` 명령은 점프 이후 복귀해야 하는 주소를 ***LR***(링크 레지스터, R14)에 자동으로 저장합니다.
...
`bl label`을 실행하면 label 위치로 점프하면서, **해당 `BL` 다음 명령어의 주소(복귀 주소)**를 LR에 저장합니다.
...
<figure style="text-align: center;">
  ...
  그럼 1. <code>BL</code> 명령어가 실행되면 PC는 서브루틴으로 이동하고, LR은 복귀할 명령어의 주소를 저장합니다. 
  </figcaption>
</figure>

## LR(Link Register) 레지스터
ARM은 함수 호출을 구현하기 위해 LR을 **복귀 주소를 저장하는 전용 레지스터**로 사용합니다.
ARMv4에서는 ***R14***가 LR로 사용됩니다.
